...
public class TypeTools {
    ...
    public TypeTools(Elements elementUtils, Types typeUtils) {
        this.parcelableType = elementUtils.getTypeElement(Constants.PARCELABLE).asType();
        this.serializableType = elementUtils.getTypeElement(Constants.SERIALIZABLE).asType();
        this.typeUtils = typeUtils;
    }

    //Element是语言元素,代表包、类、方法、变量等,但是
    //它里面没有包含自己的(类型)信息,每一个Element都指向一个TypeMirror
